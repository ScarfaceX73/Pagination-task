...
document.querySelector(".user-list").innerHTML += `<div class="obj-container">
        <div>
           <h2 class="obj-name">Name: ${name} </h2>
           <p class="obj-id"> Id: ${id} </p>
           <p class="obj-email">Email:${email} </p>
        <div>   
           </div>`

  }
          //  obj.forEach((object) => createobj(object))


const pagination= document.querySelector(".pagination");
const noOfPages = Math.ceil(obj.length/10);
...
for(let i =1;i<=noOfPages;i++){
  ...
  page.onclick=() =>{
    ...
    const pageUsers = obj.slice( (i-1)*10 ,i*10)
    document.querySelector(".user-list").innerHTML = "";
    pageUsers.forEach((object) => createobj(object));
}
  }
  const firstTenObj = obj.slice(0,10);
